...
const GenericListEditor: React.FC<GenericListEditorProps> = ({ section, data, onOpenModal, onRemoveItem, t }) => {
    const items = data || [];
    const config = sectionConfig[section];

    return (
        <SectionWrapper id={section} icon={config.icon} title={t(config.titleKey)}>
                    ...
                    {items.map((item, index) => (
                        <div key={index} className="group relative bg-blue-50 dark:bg-blue-900/30 text-primary font-medium px-3 py-1 rounded-full flex items-center gap-2">
                           ...
                           <div className="opacity-0 group-hover:opacity-100 transition-opacity flex items-center gap-1">
                                <button onClick={() => onOpenModal(section, index)} className="p-1 hover:bg-blue-100 dark:hover:bg-blue-900/50 rounded-full">
                                    <Pencil className="w-3 h-3"/>
                                </button>
                                <button onClick={() => onRemoveItem(section, index)} className="p-1 text-red-500 hover:bg-red-100 dark:hover:bg-red-900/50 rounded-full">
